...
class HuffmanSuite extends FunSuite {
  ...
  test("create code tree") {
    assert(createCodeTree('t' :: 'e' :: 'x' :: 't' :: 'x' :: 'x' :: 'x' :: Nil) === Fork(
      Fork(
        Leaf('e', 1),
        Leaf('t', 2),
        List('e', 't'),
  ...
  test("decode and [Optimal]encode frenchcode") {
    new TestTrees {
      assert(decode(Huffman.frenchCode, quickEncode(Huffman.frenchCode)("huffmanestcool".toList)) === "huffmanestcool".toList)
    }
  }
}
